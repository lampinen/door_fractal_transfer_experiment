...
```{r}
time_dependent_interaction_model = glmer(action_correct ~ isomorphic * group * session_c + trial_index_by_type_z  + (1 + session_c + correct_action | subject_id), family=binomial, data=main_data)
...
```{r}
ns_time_dependent_model = lmer(num_steps_needed~ isomorphic * (session_c) * group + trial_type + trial_index_by_type_z  + (1 + session_c | subject_id), data=num_steps_per_trial_data )
...
```{r}
dist_corr_correct_model = glm(correspondence_correct ~ fractal_hexagon_bi + total_pct_correct_z + perm_at_min_swaps, data=dist_corr_data)
