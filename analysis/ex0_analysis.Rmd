...
main_data = main_data %>%
  mutate(trial_type = factor(trial_type),
         ...
         session = scale(session, center=T, scale=F)) # center session variable
```
...
```{r}
basic_non_time_dependent_model = glmer(action_correct ~ isomorphic + group + trial_type + (1 | subject_id), family=binomial, data=main_data)
summary(basic_non_time_dependent_model)
```

Time dependent:
```{r}
basic_time_dependent_model = glmer(action_correct ~ isomorphic + session + group + trial_type + (1 + session | subject_id), family=binomial, data=main_data)
summary(basic_time_dependent_model)
```

```{r}
time_dependent_interaction_model = glmer(action_correct ~ isomorphic * session + group + trial_type + (1 + session | subject_id), family=binomial, data=main_data)
summary(basic_time_dependent_model)
```
