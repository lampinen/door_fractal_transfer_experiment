...
hacky way of doing exactly the comparison I want to do (vector-wise comparison of lists whose elements are lists).
```{r}
...
#        }
        ...
        if (trial_i == 118) { # drag drop correspondence, must undo shuffling of rooms
...
```{r}
main_data = main_data %>%
  mutate(truncated_RT = pmin(location_rts/1000,10))
```
...
```{r}
basic_non_time_dependent_model = glmer(action_correct ~ isomorphic + group + trial_type + trial_index_by_type_z + I(trial_index_by_type_z^2) +education_high + truncated_RT + (1  + correct_action | subject_id), family=binomial, data=main_data)
...
num_steps_per_trial_data = main_data %>%
  group_by(subject_id, isomorphic, group, session, session_c, trial_type, trial_index_by_type_z, education_high, start, goal) %>%
...
num_steps_summarized_data = num_steps_per_trial_data %>%
  group_by(subject_id, isomorphic, group, session, session_c, trial_type, education_high) %>%
...
num_steps_fully_summarized_data_by_type = num_steps_summarized_data  %>%
  group_by(isomorphic, group, session_c, trial_type) %>%
  summarize(num_steps_needed_se=sd(num_steps_needed)/sqrt(n()),
            num_steps_needed=mean(num_steps_needed)) %>%
  ungroup() %>%
  mutate(num_steps_needed_95ci_lower=num_steps_needed-1.96*num_steps_needed_se,
...
```{r}
ggplot(data=num_steps_fully_summarized_data_by_type %>% filter(trial_type == "door"), aes(x=session_c, y=num_steps_needed, color=isomorphic)) +
#  geom_bar(stat="identity",position="dodge") +
  scale_color_brewer(palette="Set2") +
  geom_line(size=1.2) + 
  geom_point(size=1.2) +
...
```{r}
ns_time_dependent_model_0 = lmer(num_steps_needed~ isomorphic + session_c + I(session_c^2) + group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=num_steps_per_trial_data)
ns_time_dependent_model_0.5 = lmer(num_steps_needed~ isomorphic * trial_type + session_c + I(session_c^2) + group  +  trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=num_steps_per_trial_data )
ns_time_dependent_model_1 = lmer(num_steps_needed~ isomorphic * (session_c  + trial_type)+ I(session_c^2) + group + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=num_steps_per_trial_data )
ns_time_dependent_model_1.5 = lmer(num_steps_needed~ isomorphic * (session_c ) *trial_type + I(session_c^2)  + group + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=num_steps_per_trial_data )
...
```{r}
ns_time_dependent_model_door = lmer(num_steps_needed~ isomorphic + session_c + I(session_c^2) + group  + trial_index_by_type_z + (1 + session_c | subject_id), data=num_steps_per_trial_data %>% filter(trial_type=="door"))
ns_time_dependent_model_fractal = lmer(num_steps_needed~ isomorphic + session_c + I(session_c^2) + group  + trial_index_by_type_z + (1 + session_c | subject_id), data=num_steps_per_trial_data %>% filter(trial_type=="fractal"))
```
```{r}
...
```
```{r}
ns_es_time_dependent_model_1 = lmer(num_steps_needed~ isomorphic * (trial_type * correspondence_suspect)  + session_c + I(session_c^2)+ group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data )
ns_es_time_dependent_model_1.5 = lmer(num_steps_needed~ isomorphic * (trial_type * correspondence_suspect + session_c) + I(session_c^2) + group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data )
ns_es_time_dependent_model_1.5b = lmer(num_steps_needed~ isomorphic * (trial_type + correspondence_suspect + session_c + I(session_c^2)) + group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data )
ns_es_time_dependent_model_2 = lmer(num_steps_needed~ isomorphic * (trial_type * correspondence_suspect + session_c + I(session_c^2)) + group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data )
...
ns_es_time_dependent_model_2_fractal = lmer(num_steps_needed~ isomorphic * ( correspondence_suspect + session_c + I(session_c^2)) + group  + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data  %>% filter(trial_type == "fractal"))
...
```{r}
ggplot(data=es_ns_fully_summarized_data, aes(x=session_c, y=num_steps_needed, color=isomorphic)) +
#  geom_bar(stat="identity",position="dodge") +
  scale_color_brewer(palette="Set2") +
  geom_line(size=1.2) + 
  geom_point(size=1.2) +
...
```{r}
ggplot(data=es_ns_fully_summarized_data, aes(x=session_c, y=num_steps_needed, color=correspondence_suspect)) +
#  geom_bar(stat="identity",position="dodge") +
  scale_color_brewer(palette="Set1") +
  geom_line(size=1.2) + 
  geom_point(size=1.2) +
...
## drag + drop
```{r}
num_dd_assignments_correct_model = lm(num_assignments_correct ~ isomorphic + fractal_num_steps_needed + education_high + fractal_hexagon_bi + I(rt/1000) + fractal_mean_rt, data=s2_joined_question_data %>% filter(question=="drag_drop_on_diagram"))
...
s2_joined_question_data = s2_joined_question_data %>%
  ...
  group_by(isomorphic, subject_id, fractal_hexagon_bi) %>%
  mutate(avg_num_steps_z = as.vector(scale(avg_num_steps, center=T, scale=T))) %>%
  ungroup()
```

TODO: do the z-scoring within the cells
```{r}
...
ggplot(s2_joined_question_data %>%
      ...
      aes(x=avg_num_steps, y=response))
```
...
corr_dd_model = lm(data=s2_joined_question_data %>% 
                     filter(question == "correspondence_drag_drop",
                            isomorphic),
                   num_assignments_correct ~ fractal_hexagon_bi + avg_num_steps_z + I(rt/1000) + mean_rt)
...
```{r}
dist_corr_correct_model = glm(correspondence_correct ~ fractal_hexagon_bi  + I(scale(perm_swaps_minus_perm_non_swaps))  +  avg_num_steps_z   + I(rt/1000) + mean_rt , data=dist_corr_data)
