...
ggplot(data=s2_joined_question_data %>%
  ...
  annotate("segment", x=0.5, xend=1.5, y=6.2,alpha=0.5, yend=6.2) +
  annotate("text", x=1, y= 6.4, size=10, alpha=0.5,label="*") +
  scale_fill_brewer(palette="Set2")
```
...
ggplot(distributional_similarity_full_summary, aes(x = isomorphic, y= avg_min_L1, fill=isomorphic)) +
  ...
  scale_fill_brewer(palette="Set2") +
  labs(y="Min L1 distance between policies") 
...
dist_corr_data = inner_join(s2_joined_question_data, distributional_similarity_data) %>%
  mutate(psmnps_z = as.vector(scale(perm_swaps_minus_perm_non_swaps)),
         ...
         rt=rt/1000) %>%
  group_by(isomorphic, structure, fractal_hexagon_bi) %>%
