...
Factors and centering as appropriate
```{r}
main_data = main_data %>%
  mutate(trial_type = factor(trial_type),
         group = factor(group),
         subject_id = factor(subject_id),
         session_c = scale(session, center=T, scale=F), # center session variable
...
```{r}
basic_non_time_dependent_model = glmer(action_correct ~ isomorphic + group + trial_type + trial_index_by_type_z + I(trial_index_by_type_z^2) + (1 + trial_index_by_type_z + I(trial_index_by_type_z^2)  | subject_id), family=binomial, data=main_data)
summary(basic_non_time_dependent_model)
```

Time dependent:
```{r}
basic_time_dependent_model = glmer(action_correct ~ isomorphic + session + I(session^2) + group + trial_type + trial_index_by_type_z + I(trial_index_by_type_z^2)  + (1 + session + I(session^2)  + trial_index_by_type_z + I(trial_index_by_type_z^2) | subject_id), family=binomial, data=main_data)
summary(basic_time_dependent_model)
```

```{r}
time_dependent_interaction_model = glmer(action_correct ~ isomorphic * (session + I(session^2)) + group + trial_type + trial_index_by_type_z + I(trial_index_by_type_z^2)  + (1 + session + I(session^2)  + trial_index_by_type_z + I(trial_index_by_type_z^2) | subject_id), family=binomial, data=main_data)
summary(time_dependent_interaction_model)
```

# Collapsed data for question analyses
