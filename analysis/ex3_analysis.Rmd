...
max_aligned_with_permutations = function(l1, l1_perms, l2) {
  ...
  for (perm_i in 1:length(l1_perms)) {
    prev_max_aligned = max_aligned
    max_aligned = max(max_aligned, sum(l1[l1_perms[[perm_i]]] == l2))
    if (max_aligned != prev_max_aligned) {
      max_perm = perm_i
    }  
  }
  return(c(max_aligned, max_perm))
}
```
...
Time dependent:
```{r}
basic_time_dependent_model = glmer(action_correct ~ isomorphic * group + session_c +I(session_c^2) + trial_type + trial_index_by_type_z + truncated_RT+ (1 + correct_action | subject_id), family=binomial, data=main_data)
summary(basic_time_dependent_model)
```


```{r}
time_dependent_interaction_model = glmer(action_correct ~ isomorphic *  (session_c + I(session_c^2)) * group + trial_type + trial_index_by_type_z  + truncated_RT + (1 + correct_action | subject_id), family=binomial, data=main_data)
...
```{r}
ns_time_dependent_model_0 = lmer(num_steps_needed~ isomorphic + session_c + I(session_c^2) + group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=num_steps_per_trial_data, REML=F)
ns_time_dependent_model_0.5 = lmer(num_steps_needed~ isomorphic * trial_type + session_c + I(session_c^2) + group  +  trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=num_steps_per_trial_data, REML=F )
ns_time_dependent_model_1 = lmer(num_steps_needed~ isomorphic * (session_c  + trial_type)+ I(session_c^2) + group + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=num_steps_per_trial_data , REML=F)
ns_time_dependent_model_1.5 = lmer(num_steps_needed~ isomorphic * (session_c ) *trial_type + I(session_c^2)  + group + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=num_steps_per_trial_data, REML=F )
...
```{r}
ns_time_dependent_model_door = lmer(num_steps_needed~ isomorphic + session_c + I(session_c^2) + structure + trial_index_by_type_z + I(trial_index_by_type_z^2) + avg_rt  + (1 + session_c | subject_id), data=num_steps_per_trial_data %>% filter(trial_type=="door"))
ns_time_dependent_model_fractal = lmer(num_steps_needed~ isomorphic + session_c + I(session_c^2) + structure + trial_index_by_type_z + I(trial_index_by_type_z^2) + avg_rt + (1 + session_c | subject_id), data=num_steps_per_trial_data %>% filter(trial_type=="fractal"))
...
explicit_similarity_data = question_data %>%
  filter(question %in% c("correspondence_suspect", "similarity_likert", "which_condition")) %>%
  select(subject_id, question, response) %>% 
  spread(question, response) %>%
...
```
```{r}
ns_es_time_dependent_model_1 = lmer(num_steps_needed~ isomorphic * (trial_type * correspondence_suspect)  + session_c + I(session_c^2)+ group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data , REML=F)
ns_es_time_dependent_model_1.5 = lmer(num_steps_needed~ isomorphic * (trial_type * correspondence_suspect + session_c) + I(session_c^2) + group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data, REML=F )
ns_es_time_dependent_model_1.5b = lmer(num_steps_needed~ isomorphic * (trial_type + correspondence_suspect + session_c + I(session_c^2)) + group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data, REML=F )
ns_es_time_dependent_model_2 = lmer(num_steps_needed~ isomorphic * (trial_type * correspondence_suspect + session_c + I(session_c^2)) + group  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data, REML=F )
...
ns_es_time_dependent_model_1_fractal = lmer(num_steps_needed~ isomorphic * ( correspondence_suspect) + session_c + I(session_c^2)+ structure  + trial_index_by_type_z + I(trial_index_by_type_z^2)  + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data  %>% filter(trial_type == "fractal"))
...
es_ns_fully_summarized_data = es_ns_summarized_data  %>%
  group_by(isomorphic, group, session_c, correspondence_suspect) %>%
  summarize(num_steps_needed_se=sd(num_steps_needed)/sqrt(n()),
            num_steps_needed=mean(num_steps_needed)) %>%
  ungroup() %>%
  mutate(num_steps_needed_95ci_lower=num_steps_needed-1.96*num_steps_needed_se,
...
## drag + drop
```{r}
num_dd_assignments_correct_model = lm(num_assignments_correct ~ fractal_num_steps_needed + fractal_Flipping, data=s2_joined_question_data %>% filter(question=="drag_drop_on_diagram"))
...
```
```{r}
xtabs(~ ds_correct + isomorphic, s2_joined_question_data)
```

```{r}
ggplot(data=s2_joined_question_data %>%
         filter(!is.na(ds_correct)) %>%
         group_by(isomorphic, fractal_Flipping) %>%
         summarize(pct_correct=mean(ds_correct)) %>%
         ungroup(),
       aes(x=isomorphic, y = pct_correct, fill=isomorphic)) +
  geom_bar(stat="identity") +
  facet_wrap(~ fractal_Flipping)
```

```{r}
ds_correct_model = glm(ds_correct ~ isomorphic + fractal_Flipping + avg_num_steps_z, data=s2_joined_question_data)
...
ggplot(s2_joined_question_data %>%
         filter(question == "similarity_likert"),
      aes(x=avg_num_steps_z, y=response, color=isomorphic)) +
  geom_point() +
  xlim(-3, 3)
...
```{r}
correspondence_correct_model_2 = glm(correspondence_correct ~ fractal_hexagon_bi + avg_num_steps + subject_pct_correct_correlation, data=cc_data)
...
corr_dd_model = lm(data=s2_joined_question_data %>% 
                     filter(question == "correspondence_drag_drop",
                            isomorphic),
                   num_assignments_correct ~ fractal_hexagon_bi + avg_num_steps_z)
...
```
```{r}
dist_sim_model = lm(min_L1 ~ isomorphic * fractal_Flipping, data=distributional_similarity_data)
summary(dist_sim_model)
```

```{r}
ggplot(distributional_similarity_full_summary, aes(x = isomorphic, y= avg_min_L1, fill=isomorphic)) +
  geom_bar(stat="identity") +
...
```{r}
ggsave("figures/ex3/distributional_similarities_by_group.png")
```
...
```{r}
dist_corr_correct_model = glm(correspondence_correct ~ psmnps_z +  avg_num_steps_z   + rt, data=dist_corr_data)
summary(dist_corr_correct_model)
```


```{r}
ggplot(data=dist_corr_data %>% filter(!is.na(correspondence_correct)), aes(x=I(scale(perm_swaps_minus_perm_non_swaps)), y=1-correspondence_correct)) +
  geom_point(aes(color=fractal_Flipping))  + 
  geom_smooth(method="glm", method.args=list(family="binomial"), se=F)
```

```{r}
coefficient_getter = function(data, indices) {
  res = glm(correspondence_correct ~ psmnps_z +  avg_num_steps_z   + rt, data=data[indices,])
  return(res$coefficients)
}
```

```{r}
bootstrap_results = boot(data=dist_corr_data, statistic=coefficient_getter, R=1000)
```
...
```
All but RT remain significant at 95% confidence
...
```{r}
es_dist_corr_correct_model = glm(correspondence_correct ~ fractal_hexagon_bi  + I(scale(perm_swaps_minus_perm_non_swaps)) * correspondence_suspect +  avg_num_steps_z   + I(rt/1000) + mean_rt , data=es_dist_corr_data)
summary(es_dist_corr_correct_model)
```


# predicting from drag and drop
```{r}
es_dist_corr_data_2 = es_dist_corr_data %>%
  filter(two_corr_Qs & question %in% c("correspondence_identify", "correspondence_drag_drop")) %>%
  group_by(subject_id) %>%
  mutate(best_dd_perm = mean(best_dd_perm, na.rm=T), # hacky way of spreading this value to all questions
...
```{r}
dist_corr_correct_model_b = glm(correspondence_correct ~ fractal_hexagon_bi  + I(scale(perm_swaps_minus_perm_non_swaps)) + best_dd_perm_flips +  avg_num_steps_z   + I(rt/1000) + mean_rt , data=es_dist_corr_data_2)
summary(dist_corr_correct_model_b)
```
Hard to say because of lack of power to detect the effect at all
```{r}
dist_corr_correct_model_b0 = glm(correspondence_correct ~ I(scale(perm_swaps_minus_perm_non_swaps)), data=es_dist_corr_data_2)
...
corr_dd_prediction_model = glm(data=es_dist_corr_data_2 %>% 
                     filter(question == "correspondence_drag_drop", isomorphic),
                     best_dd_equals_perm_at_min ~ fractal_hexagon_bi + avg_num_steps_z)
summary(corr_dd_prediction_model)
```

```{r}
xtabs(~ best_dd_perm + perm_at_min, data=es_dist_corr_data_2 %>%
        filter(question=="correspondence_drag_drop"))
```
