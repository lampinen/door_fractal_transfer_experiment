...
```{r}
num_dd_assignments_correct_model = lm(corrected_nac ~ isomorphic  * fractal_Flipping + correspondence_suspect + avg_num_steps_fractal_z, data=s2_joined_question_data %>% filter(question=="drag_drop_on_diagram"))
...
```{r}
dist_corr_correct_model = glm(correspondence_correct ~ psmnps_z  
