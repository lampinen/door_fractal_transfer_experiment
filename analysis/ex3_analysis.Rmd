...
Time dependent:
```{r}
basic_time_dependent_model = glmer(action_correct ~ isomorphic * group + session_c +I(session_c^2) + trial_type + trial_index_by_type_z + I(pmin(location_rts/1000,10))  + (1 + correct_action | subject_id), family=binomial, data=main_data)
summary(basic_time_dependent_model)
```


```{r}
time_dependent_interaction_model = glmer(action_correct ~ isomorphic *  (session_c + I(session_c^2)) + group + trial_type + trial_index_by_type_z  + I(pmin(location_rts/1000,10)) + (1 + correct_action | subject_id), family=binomial, data=main_data)
...
```
```{r}
ns_time_dependent_model = lmer(num_steps_needed~ isomorphic * (session_c + I(session_c^2) + correspondence_suspect + group)  + trial_type + trial_index_by_type_z + avg_rt + (1 + session_c | subject_id), data=es_joined_ns_data )
summary(ns_time_dependent_model)
```
