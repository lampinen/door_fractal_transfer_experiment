...
## drag + drop
```{r}
num_dd_assignments_correct_model = lm(num_assignments_correct ~ avg_num_steps_fractal_z + fractal_Flipping + isomorphic, data=s2_joined_question_data %>% filter(question=="drag_drop_on_diagram"))
summary(num_dd_assignments_correct_model)
```
```{r}
ggplot(data=s2_joined_question_data %>%
         filter(question == "drag_drop_on_diagram"),
       aes(x=-avg_num_steps_fractal_z, y=num_assignments_correct))+
  geom_point(aes(color=isomorphic)) +
  geom_smooth(method="lm", color="black") +
  labs(x="Performance (reverse number of steps, Z-scored within condition)", y="Number of fractals correctly assigned") +
  scale_color_brewer(palette="Set2")
```

```{r}
ggsave("../writing/talks/figures/ex3/ds_by_ns.png", width=7, height=5)
```
...
ggplot(data=s2_joined_question_data %>%
         filter(question == "drag_drop_on_diagram") %>%
         group_by(isomorphic) %>%
         summarize(nac = mean(num_assignments_correct), nac_se=sd(num_assignments_correct)/sqrt(n()),
...
```{r}
sl_model = lm(response ~  isomorphic , data=s2_joined_question_data %>%
...
```{r}
oth_model = lm(response ~  isomorphic  + fractal_hexagon_bi, data=s2_joined_question_data %>%
...
```{r}
ggplot(data=s2_joined_question_data %>%
         filter(!is.na(which_condition_correct)), aes(x=-avg_num_steps_z, y=1*which_condition_correct)) +
  geom_point() +
  labs(x="Performance (reverse number of steps, Z-scored within condition)", y="Correct about same/different structures") +
...
```{r}
dist_corr_correct_model = glm(correspondence_correct ~ psmnps_z  +  avg_num_steps_z   + I(rt/1000), family="binomial", data=dist_corr_data)
summary(dist_corr_correct_model)
```

```{r}
dist_corr_correct_model_0 = glm(correspondence_correct ~ perm_at_min_swaps  +  avg_num_steps_z   + rt, family="binomial", data=dist_corr_data)
summary(dist_corr_correct_model_0)
```

```{r}
dist_corr_correct_model_2 = glm(correspondence_correct ~ perm_at_min_swaps + psmnps_z +  avg_num_steps_z   + rt, family="binomial", data=dist_corr_data)
...
ggplot(data=dist_corr_data %>% filter(!is.na(correspondence_correct)), aes(x=I(scale(perm_swaps_minus_perm_non_swaps)), y=1-correspondence_correct)) +
  ...
  geom_smooth(method="glm", method.args=list(family="binomial"), color="black", se=F)
```
...
```{r}
es_dist_corr_correct_model = glm(correspondence_correct ~ fractal_hexagon_bi  + I(scale(perm_swaps_minus_perm_non_swaps)) * correspondence_suspect +  avg_num_steps_z   + I(rt/1000) + mean_rt , family="binomial", data=es_dist_corr_data)
...
corr_dd_prediction_model = glm(data=es_dist_corr_data_2 %>% 
                     ...
                     perm_at_min_in_best_dd_perms ~ 1)
