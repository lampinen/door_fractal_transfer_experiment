...
ggplot(data=s2_joined_question_data %>%
  ...
  labs(x="Avg. number of steps needed", y="Number of fractals correctly assigned") 
...
ggplot(data=s2_joined_question_data %>%
  ...
  geom_hline(aes(yintercept=2.505), linetype=2, alpha=0.5)+
  labs(y="Number of fractals correctly assigned") +
  annotate("text", x=2, y= 5.8, size=10, alpha=0.5, label="***") +
  annotate("text", x=1, y= 5.8, size=10,alpha=0.5, label="***") +
  annotate("segment", x=1, xend=2, y=6.2,alpha=0.5, yend=6.2) +
  annotate("text", x=1.5, y= 6.4, size=6, alpha=0.5,label="ns") +
...
ggplot(data=s2_joined_question_data %>%
  ...
  annotate("segment", x=1, xend=2, y=6.2,alpha=0.5, yend=6.2) +
  annotate("text", x=1.5, y= 6.4, size=10, alpha=0.5,label="*") +
  annotate("segment", x=0.75, xend=1.25, y=6.4,alpha=0.5, yend=6.4) +
  annotate("text", x=1, y= 6.6, size=10, alpha=0.5,label="*") +
  annotate("segment", x=1.75, xend=2.25, y=6.4,alpha=0.5, yend=6.4) +
  annotate("text", x=2, y= 6.7, size=6, alpha=0.5,label="ns") +
  scale_fill_brewer(palette="Set2")
```
...
ggplot(data=s2_joined_question_data %>%
  ...
  annotate("text", x=2, y= 1, size=10, alpha=0.5, label="**") +
  annotate("text", x=1, y= 1.03, size=6,alpha=0.5, label="ns") +
  annotate("segment", x=1, xend=2, y=1.1,alpha=0.5, yend=1.1) +
  annotate("text", x=1.5, y= 1.15, size=6, alpha=0.5,label="ns") +
  scale_fill_brewer(palette="Set2") 
...
```{r}
dist_sim_model = lm(avg_num_steps ~ isomorphic  *  (min_L1) +  structure, data=dist_corr_data)
summary(dist_sim_model)
```
...
```{r}
ggplot(data=dist_corr_data, aes(x=min_L1, y=avg_num_steps, color=isomorphic)) +
  geom_point() +
  geom_smooth(method="lm") +
  scale_color_brewer(palette="Set2") +
...
ggplot(data=dist_corr_data %>%
       ...
       aes(x=min_L1, y=avg_num_steps, color=isomorphic)) +
  geom_point() +
  geom_smooth(method="lm") +
  scale_color_brewer(palette="Set2") +
...
dist_corr_data = inner_join(dist_corr_data, explicit_similarity_data %>%
                              select(subject_id, correspondence_suspect, one_task_helpful_for_other, similarity_likert, which_condition, ever_noticed)) %>%
...
```{r}
sl_model = lm(response ~  isomorphic * min_L1_z + structure, data=dist_corr_data %>%
                filter(question == "similarity_likert"))
summary(sl_model)
```
...
ggplot(data=es_dist_corr_data_2 %>%
         ...
         ungroup(),
        aes(x=isomorphic, y=perm_at_min_alignment_with_best_dd_perms, fill=isomorphic)) +
  geom_bar(stat="identity", position="dodge") +
  geom_errorbar(aes(ymin=ci95_lower, ymax=ci95_upper), width=0.25, position=position_dodge()) +
  scale_fill_brewer(palette="Set2") 
```

# bootstrapping null distribution

```{r}
cdd_data = es_dist_corr_data_2 %>%
  filter(question=="correspondence_drag_drop",
         isomorphic == "TRUE") %>%
  select(subject_id, fractal_hexagon_bi, perm_at_min, best_dd_perms)
```

```{r}
null_dist_avg_alignments = c()
num_permutations = 5000
for (i in 1:num_permutations) {
  this_data = cdd_data %>%
    mutate(perm_at_min = sample(perm_at_min)) %>%
    group_by(subject_id) %>%
    mutate(
           perm_at_min_alignment_with_best_dd_perms=ifelse(fractal_hexagon_bi, # permute perm_at_min values
                                                           max_aligned_with_permutations(c(1,2,3,4,5,6), hb_perm_locs[best_dd_perms[[1]]], unlist(hb_perm_locs[perm_at_min]))[1],
                                                           max_aligned_with_permutations(c(1,2,3,4,5,6), ht_perm_locs[best_dd_perms[[1]]], unlist(ht_perm_locs[perm_at_min]))[1])) %>%
    ungroup() %>%
    summarize(perm_at_min_alignment_with_best_dd_perms=mean(perm_at_min_alignment_with_best_dd_perms))
  null_dist_avg_alignments = c(null_dist_avg_alignments, this_data$perm_at_min_alignment_with_best_dd_perms)
}
```
