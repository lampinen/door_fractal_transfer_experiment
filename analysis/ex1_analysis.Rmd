...
```{r}
ggplot(data=collapsed_main_data, aes(x=session, y=pct_correct, linetype=trial_type, color=isomorphic)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept=0.5, alpha=0.75, linetype=2) +
#  ylim(0,1) +
...
```{r}
pc_model = lmer(fractal_pct_correct ~ door_pct_correct * isomorphic + (1|subject_id), data=pcc_data)
summary(pc_model)
```
