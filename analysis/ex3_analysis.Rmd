...
num_steps_per_trial_data = main_data %>%
  group_by(subject_id, isomorphic, group, session_c, trial_type, trial_index_by_type_z, education_high, start, goal) %>%
...
num_steps_summarized_data = num_steps_per_trial_data %>%
  group_by(subject_id, isomorphic, group, session_c, trial_type, education_high) %>%
...
## drag + drop
```{r}
num_dd_assignments_correct_model = lm(num_assignments_correct ~ isomorphic * fractal_pct_correct + education_high + fractal_hexagon_bi + I(rt/1000) + fractal_mean_rt, data=s2_joined_question_data %>% filter(question=="drag_drop_on_diagram"))
...
```{r}
ds_correct_model = glm(ds_correct ~ fractal_hexagon_bi + isomorphic * fractal_pct_correct  + I(rt/1000) + fractal_mean_rt, data=s2_joined_question_data)
...
```{r}
sl_model = lm(response ~  isomorphic * total_pct_correct_z + I(rt/1000) + mean_rt, data=s2_joined_question_data %>%
...
```{r}
oth_model = lm(response ~  isomorphic * (total_pct_correct_z)  + fractal_hexagon_bi  + I(rt/1000) + mean_rt, data=s2_joined_question_data %>%
...
```{r}
correspondence_correct_model = glm(correspondence_correct ~ fractal_hexagon_bi + total_pct_correct_z + I(rt/1000) + mean_rt, data=s2_joined_question_data)
...
```{r}
ever_noticed_model = glm(ever_noticed ~ fractal_hexagon_bi + isomorphic * education_high  + I(rt/1000) + mean_rt+  total_pct_correct_z, data=s2_joined_question_data)
summary(ever_noticed_model)
```

```{r}
when_noticed_model = lm(when_noticed~  isomorphic * total_pct_correct_z  + I(rt/1000) + mean_rt , data=s2_joined_question_data %>%
...
```{r}
dist_corr_correct_model = glm(correspondence_correct ~ fractal_hexagon_bi  + (perm_swaps_minus_perm_non_swaps)  + I(rt/1000) + mean_rt + total_pct_correct_z , data=dist_corr_data)
