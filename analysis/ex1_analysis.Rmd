...
```{r}
ns_time_dependent_model = lmer(num_steps_needed~ isomorphic * group * (session_c + I(session_c^2))  + trial_type + trial_index_by_type_z + I(trial_index_by_type_z^2)  + (1 + session_c | subject_id), data=num_steps_per_trial_data)
...
```{r}
sl_model = lm(response ~  isomorphic + total_pct_correct, data=s2_joined_question_data %>%
...
```{r}
oth_model = lm(response ~  isomorphic * total_pct_correct + fractal_square_cycles, data=s2_joined_question_data %>%
