...
```{r}
num_dd_assignments_correct_es_model = lm(corrected_nac ~ isomorphic  * (correspondence_suspect + structure) + avg_num_steps_z, data=s2_joined_question_data %>% filter(question=="drag_drop_on_diagram"))
summary(num_dd_assignments_correct_es_model)
```
...
```{r}
num_dd_assignments_correct_es_model = lm(corrected_nac ~ isomorphic  * (scale(as.numeric(one_task_helpful_for_other), scale=F) + structure) + avg_num_steps_z, data=s2_joined_question_data %>% filter(question=="drag_drop_on_diagram"))
summary(num_dd_assignments_correct_es_model)
```
```{r}
ggplot(data=s2_joined_question_data %>%
...
```{r}
ggsave("../writing/talks/figures/ex3/nac_by_iso_group_oth.png", width=7, height=5)
```
...
```{r}
ds_correct_model = glm(ds_correct ~ isomorphic * as.numeric(similarity_likert), family="binomial", data=s2_joined_question_data)
summary(ds_correct_model)
```
...
```{r}
ds_correct_model = glm(ds_correct ~ isomorphic * scale(as.numeric(one_task_helpful_for_other)), family="binomial", data=s2_joined_question_data)
summary(ds_correct_model)
```
