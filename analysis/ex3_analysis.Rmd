...
```
```{r}
ns_es_time_dependent_model_3_fractal = lmer(num_steps_needed~ isomorphic * I(as.numeric(one_task_helpful_for_other)-2) + session_c + I(session_c^2)+ structure  + trial_index_by_type_z + I(trial_index_by_type_z^2)  + avg_rt_on_trial + (1 + session_c | subject_id), data=es_joined_ns_data  %>% filter(trial_type == "fractal"))
summary(ns_es_time_dependent_model_3_fractal)
```
...
ggsave("../writing/talks/figures/ex3/ns_by_correspondence_suspect.png")
```
...
ggplot(data=s2_joined_question_data %>%
         ...
         group_by(isomorphic) %>%
         summarize(pct_correct=mean(ds_correct), pct_correct_se=sqrt(n()*pct_correct * (1-pct_correct)),
                   pct_correct_95ci_lower=pct_correct - 1.96*pct_correct_se,
                   pct_correct_95ci_upper=pct_correct + 1.96*pct_correct_se) %>%
         ungroup(),
       aes(x=isomorphic, y = pct_correct, fill=isomorphic)) +
  geom_bar(stat="identity") +
  #geom_errorbar(aes(ymin=pct_correct_95ci_lower, ymax=pct_correct_95ci_upper)) +
  scale_fill_brewer(palette="Set2") 
#  facet_wrap(~ fractal_Flipping)
```

```{r}
ds_correct_model = glm(ds_correct ~ isomorphic + fractal_Flipping + avg_num_steps_fractal_z, data=s2_joined_question_data)
...
```{r}
sl_model = lm(response ~  isomorphic + avg_num_steps_fractal_z, data=s2_joined_question_data %>%
...
ggplot(s2_joined_question_data %>%
         filter(question == "similarity_likert"),
      aes(x=-avg_num_steps_fractal_z, y=response, color=isomorphic)) +
  geom_point() +
  labs(x="Z-scored performance on task B")
  xlim(-3, 3)
...
```{r}
oth_model = lm(response ~  isomorphic * (avg_num_steps_fractal_z)  + fractal_hexagon_bi, data=s2_joined_question_data %>%
...
```{r}
dist_corr_correct_model_0 = glm(correspondence_correct ~ perm_at_min_swaps  +  avg_num_steps_z   + rt, data=dist_corr_data)
summary(dist_corr_correct_model_0)
```

```{r}
dist_corr_correct_model_2 = glm(correspondence_correct ~ perm_at_min_swaps + psmnps_z +  avg_num_steps_z   + rt, data=dist_corr_data)
...
corr_dd_prediction_model = glm(data=es_dist_corr_data_2 %>% 
                     ...
                     perm_at_min_in_best_dd_perms ~ fractal_hexagon_bi + avg_num_steps_z)
summary(corr_dd_prediction_model)
```

```{r}
xtabs(~ perm_at_min_in_best_dd_perms +fractal_Flipping, data=es_dist_corr_data_2 %>%
        filter(question=="correspondence_drag_drop"))
```
